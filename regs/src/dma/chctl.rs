use crate::dma::{AddressGenMode, Bank, Channel, PriorityLevel, TransferDirection, TransferSize};
...
impl RegisterBlock {
    ...
    pub fn transfer_direction(&self) -> Option<TransferDirection> {
        TransferDirection::from_value((self.bits >> 4) & 0x1)
    }
...
impl RegisterBlock {
    ...
    pub fn set_priority_level(&mut self, value: PriorityLevel) {
        self.bits = (self.bits & !(0x2 << 12)) | (value.raw_value() << 12);
    }

    #[inline(always)]
    pub fn set_mem_transfer_size(&mut self, value: TransferSize) {
        self.bits = (self.bits & !(0x2 << 10)) | (value.raw_value() << 10);
    }

    #[inline(always)]
    pub fn set_peri_transfer_size(&mut self, value: TransferSize) {
        self.bits = (self.bits & !(0x2 << 8)) | (value.raw_value() << 8);
    }
